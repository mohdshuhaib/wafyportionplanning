    ...
    {data.subjects.length===0 ? <EmptySubjects semester={semester} onAdd={openAdd}/> : <div className="grid items-start gap-5 lg:grid-cols-[300px_1fr]">
      ...
      {subject && <section className="space-y-5">
        <div className="card p-5 md:p-6">
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-[1.1fr_2fr]"><div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-4"><p className="text-[10px] font-extrabold uppercase tracking-widest text-emerald-700">Usthad's target per period</p><p className="display mt-1 text-3xl font-semibold text-emerald-900">{formatNumber(subject.pages_per_period)} <span className="font-sans text-sm font-bold">pages</span></p></div><div><div className="flex items-center justify-between gap-3"><p className="eyebrow">Full semester status</p><span className={`pill ${statusStyle(stats.planStatus)}`}>{stats.planStatus}</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-stone-100"><motion.div initial={{width:0}} animate={{width:`${Math.min(100,stats.pct)}%`}} className="h-full rounded-full bg-[#16775d]"/></div><div className="mt-4 grid grid-cols-1 gap-2 text-center sm:grid-cols-3"><Stat label="Periods" value={`${formatNumber(stats.periods)}/${formatNumber(stats.targetPeriods)}`}/><Stat label="Pages" value={`${formatNumber(stats.pages)}/${formatNumber(stats.targetPages)}`}/><Stat label="Progress" value={`${stats.pct}% / 100%`}/></div></div></div>
        </div>
...
function Stat({label,value}:{label:string;value:string|number}){return <div className="min-w-0 rounded-2xl bg-[#f3f6f2] p-3"><strong className="display block break-words text-base leading-tight sm:text-lg lg:text-xl">{value}</strong><span className="muted block truncate text-[10px] font-bold uppercase tracking-wide">{label}</span></div>;}
